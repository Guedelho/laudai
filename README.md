...
| `/home`                          | Public marketing landing page (hero · especialistas · como funciona · recursos · segurança · preço · faq · contato · CTA) — proxy rewrites `laudai.vet/` here (app stays on subdomain). SEO metadata + canonical + Organization/WebSite/SoftwareApplication JSON-LD; `robots.ts` · `sitemap.ts` · `llms.txt` served on the marketing host |
