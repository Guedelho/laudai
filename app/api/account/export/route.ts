...
export const GET = withApiHandler(async ({ userId, admin }) => {
  const [profileRes, reportsRes, petsRes, clientsRes, vetsRes, consentsRes, imagesRes, auditRes] = await Promise.all([
  ...
  return new NextResponse(JSON.stringify(payload, null, 2), {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="laudai-export-${userId}-${brazilToday()}.json"`,
    },
  });
});
