...
- **Next.js 16 App Router** — Tailwind 4 (PostCSS-based, no `tailwind.config.*`), Supabase SSR via `@supabase/ssr`. `cacheComponents: true`. The app uses the **stock Tailwind type scale** (16px base) — a previous 12px `@theme` downscale was reverted because it made the whole UI too small; do not reintroduce a global text rescale. Never use arbitrary `text-[Npx]` values — pick a scale step. The UI font is Arial (set in `globals.css`; no webfont in the root layout). The four signature-preview script fonts load via `next/font/google` inside `ProfileForm.tsx` (self-hosted, no injected stylesheet); the PDF fetches its own fonts separately in `lib/report/pdf.ts`.
