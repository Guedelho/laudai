...
- **Next.js 16 App Router** — Tailwind 4 (PostCSS-based, no `tailwind.config.*`), Supabase SSR via `@supabase/ssr`. `cacheComponents: true`. `app/globals.css` overrides the Tailwind text scale in `@theme` to a **12px base** (`--text-base: 12px`, `xs…6xl` rescaled with paired line-heights) and sets `body { font-size: var(--text-base) }` — so `text-*` utilities are ~0.75× their stock sizes app-wide; spacing, `max-w-*`, and breakpoints stay rem-default so layout/responsive behavior is unchanged. Never use arbitrary `text-[Npx]` values — pick a scale step. The UI font is Arial (set in `globals.css`; no webfont in the root layout). The four signature-preview script fonts load via `next/font/google` inside `ProfileForm.tsx` (self-hosted, no injected stylesheet); the PDF fetches its own fonts separately in `lib/report/pdf.ts`.
